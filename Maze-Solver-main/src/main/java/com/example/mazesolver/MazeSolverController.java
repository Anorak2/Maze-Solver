...
public class MazeSolverController implements Initializable {
    ...
    @FXML
    GridPane mazeGridPane, backgroundGridPane;
    @FXML
    Circle playerCircle;
    @FXML
    Slider speedSlider;
    ...
    private void displayMaze(){
        mazeGridPane.getChildren().clear();
        backgroundGridPane.getChildren().clear();
        for(int x = 0; x < 8; x++){
            RowConstraints row = new RowConstraints(tileSize-1);
            //mazeGridPane.getRowConstraints().add(row);
            backgroundGridPane.getRowConstraints().add(row);

            ColumnConstraints column = new ColumnConstraints(tileSize-1);
            //mazeGridPane.getColumnConstraints().add(column);
            backgroundGridPane.getColumnConstraints().add(column);
        }
        ...
        for(int row = 0; row < gridSize; row++) {
            for (int col = 0; col < gridSize; col++) {
                if(mainMaze[row][col].isWall) {
                    ...
                    String x = roundCornersWall(row, col);
                    newRectangle.setStyle(x);
                    newRectangle.setPrefSize(tileSize, tileSize);
                    mazeGridPane.add(newRectangle, col, row);
                }
                if(!mainMaze[row][col].isWall){
                    Region newRectangle = new Region();
                    newRectangle.setPrefSize(tileSize, tileSize);
                    //newRectangle.setStyle(roundCornersSpace(row, col));
                    mazeGridPane.add(newRectangle, col, row);
                }
    ...
    private final int roundnessValue = 10;
    private String roundCornersWall(int row, int col){
    ...
    private String roundCornersSpace(int row, int col){
        StringBuilder build = new StringBuilder();
        build.append("-fx-background-color: #1f262a;");
        int[] roundness = {0, 0, 0, 0};
        boolean triggered = false;
        //up left
        if(row-1 >= 0 && col-1 >= 0){
            if(mainMaze[row-1][col].canMoveHere && mainMaze[row][col-1].canMoveHere && mainMaze[row-1][col-1].canMoveHere){
                roundness[0] = roundnessValue;
                triggered = true;
            }
        }
        ...
        if(triggered){
            ...
            rect.setStyle("-fx-background-color: #20bf55;");
            rect.setPrefSize(tileSize, tileSize);
            backgroundGridPane.add(rect, col, row);
        }

        build.append("-fx-background-radius:");
